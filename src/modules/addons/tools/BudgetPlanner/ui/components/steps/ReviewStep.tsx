...
export default function ReviewStep() {
  ...
  const income = (v.net_monthly_income || 0) + (v.other_income || 0);
  const fixed = v.fixed ? Object.values(v.fixed).reduce((a: number, b: number) => a + (b || 0), 0) : 0;
  const variable = v.variable ? Object.values(v.variable).reduce((a: number, b: number) => a + (b || 0), 0) : 0;
  const mins = [...(v.debts?.credit_cards || []), ...(v.debts?.loans || [])]
    .reduce((a, d) => a + (d.min_payment || 0), 0);
...
function Stat({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className={`rounded-md border p-3 ${highlight ? "bg-emerald-50 dark:bg-emerald-950/20" : "bg-white "}`}>
      <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{label}</div>
      <div className="text-base font-semibold tabular-nums">${value.toFixed(2)}</div>
    </div>
  );
}
